...
def display():
    ...
    render_text(10, window_height - 20, f"Scaling | Axis: {axis} | Shape: {shape_index+1} | Proj: {projection}")
    render_text(10, window_height - 40, f"S({sx:.2f},{sy:.2f},{sz:.2f})")
...
def special_keys(key, x, y):
    ...
    delta_s = 0.1
    inc = key == GLUT_KEY_UP
    val = (delta_s if inc else -delta_s)
    if axis == "x":
        sx += val
    elif axis == "y":
        sy += val
...
def main():
    ...
    glutCreateWindow(b"Lab 5 - Scaling")
    init_gl()
